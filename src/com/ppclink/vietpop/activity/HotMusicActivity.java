...
public class HotMusicActivity extends Activity implements DataInterface, FileDownloadInterface{
	
	private static final String TAG = "HotMusicActivity";
	

	private ListView list_topic = null;
    private ListView list_song = null;
    ...
    private ArrayList<FileDownload> downloaders;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		if (DisplayMode.mode == 1) setContentView(R.layout.hot_music_320x480);
		else if (DisplayMode.mode == 2) setContentView(R.layout.hot_music_480x800);         
        ...
        list_topic.setVisibility(View.VISIBLE);
        ...
        if (null == ni){
        	...
        	Toast.makeText(getApplicationContext(), "Please turn-on network connection", Toast.LENGTH_LONG);
        } else if (ni.getState() == NetworkInfo.State.CONNECTED){            
        	Log.d(TAG, "network connected");
            progressDialog = new ProgressDialog(HotMusicActivity.this);
    		progressDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
    		progressDialog.setMessage("Đang tải chủ đề...");
    		progressDialog.show();
            callDataTask(Constant.TOPIC, null);               
	...
	public Dialog onCreateDialog(int id) {    	
				...
				public void onClick(DialogInterface dialog, int which) {
					if (which < temp3.size() - 1){
						...
						if (listDelete.size() != 0){
							Toast.makeText(HotMusicActivity.this, "Thêm thành công. \nCó " + listDelete.size()+" bài đã tồn tại trong playlist.", 2000).show();
	    				}
						else{
							Toast.makeText(HotMusicActivity.this, "Thêm thành công.", 1500).show();
						}
						dialog.cancel();
					}
					if (which == temp3.size() - 1) {
      	...
      	case CREATE_PLAYLIST_ALL_SONG:           
      	 ...
      	 case ERROR:
      		 ...
      		 break;
      	case SONG_OPTION:
   		 final CharSequence[] items = {"Thêm vào danh sách", "Thêm tất cả vào danh sách", "Tìm Lyric bài này"};
			...
			public void onClick(DialogInterface dialog, int item) {
				...
				case 2:
					...
					lyric_Task.delegate = HotMusicActivity.this;
      	...
      	case SHOW_LYRIC:
			...
			AlertDialog alert4;
    		 ...
    		 builder4.setSingleChoiceItems(test, -1, new DialogInterface.OnClickListener() {				
				public void onClick(DialogInterface dialog, int which) {
					progressDialog = new ProgressDialog(HotMusicActivity.this);
					progressDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
    ...
    class TopicAdapter extends BaseAdapter {
        private List<BaseData> listTopic;        
        ...
        public TopicAdapter(Context context, List<BaseData> listTopic) {
    	...
    	public View getView(int position, View convertView, ViewGroup parent) {
    		final BaseData entry = listTopic.get(position);
    		TopicHolder holder;    		
    		if (convertView == null) {
    		...
    		convertView.setBackgroundResource((position & 1) == 1 ? mBackground1 : mBackground2);    		
    			...
    			holder.textTopic.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						if (state == 0) {
    						callDataTask(Constant.SONG_BY_TOPIC, entry.getLink());    						
    						progressDialog = new ProgressDialog(HotMusicActivity.this);
    						progressDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
       ...
       public SongAdapter(Context context, List<Song> listSong) {
    	...
    	public View getView(final int position, View convertView, ViewGroup parent) {
    		...
    		if (convertView == null) {
                 ...
                 holder.textSong = (TextView) convertView.findViewById(R.id.text_song);
                 holder.textArtist = (TextView)convertView.findViewById(R.id.text_artist);
                 holder.tvDownloadInfo = (TextView)convertView.findViewById(R.id.tvDownload);
                 holder.icon1 = (ImageButton) convertView.findViewById(R.id.icon1);
                 holder.ibDownload = (ImageButton)convertView.findViewById(R.id.icon2);
                 holder.ibCancel = (ImageButton)convertView.findViewById(R.id.ibCancelDownload);
    		...
    		convertView.setBackgroundResource((position & 1) == 1 ? mBackground1 : mBackground2);                    		
    				...
    				public void onClick(View v) {
						...
						if (cancel != -1){
							if (downloaders.get(cancel).getState() == DownloadState.DOWNLOAD_INPROGRESS){
								...
								holder.ibCancel.setVisibility(alControlView.get(position)+8);
								holder.textArtist.setVisibility(alControlView.get(position));
								holder.ibDownload.setVisibility(alControlView.get(position));
							}
							holder.ibDownload.setImageResource(mIcon2);
						}
					}
    			...
    			holder.ibDownload.setOnClickListener(new OnClickListener() {					
					public void onClick(View v) {
						final File fileDownload = new File(Constant.PATH_DOWNLOAD, entry.getName()+".mp3");
						if (fileDownload.exists()){
							// Xoa bai hat khoi may
							new AlertDialog.Builder(HotMusicActivity.this)
							.setTitle("Delete")
							...
							if (isDownload){
								FileDownload downloader = new FileDownload(entry.getName(), entry.getLink(), Constant.PATH_DOWNLOAD);						
								downloader.delegate = HotMusicActivity.this;
								downloader.setTag(position);
								downloader.init(holder.pbDownload, holder.tvDownloadInfo, holder.ibCancel, holder.ibDownload, holder.textArtist);
    ...
    public void callDataTask(int id, String url) {
        ...
        if (id == Constant.TOPIC) {
        	dataReader.getAllTopic();
        }
        else if (id == Constant.SONG_BY_TOPIC) dataReader.getAllSongByTopic(url);
	}
    ...
    public void applyRotationADV(int position, float start, float end) {
        ...
        final TranslateADV rotation =
                new TranslateADV(start, end, centerX, centerY);
    ...
    private final class DisplayNextViewADV implements Animation.AnimationListener {
        private final int mPosition;
	...
	public int onComplete(Integer result) {
		...
		if (result == Constant.TOPIC) {
			...
			if (temp == null){
				...
				list_topic.setAdapter(new TopicAdapter(this, temp));
				return 0; 
			}
		}
		if (result == Constant.SONG_BY_TOPIC) {
			List<BaseData> temp = null;
			temp = dataReader.data;
			if (temp == null) {
				showDialog(ERROR);
		...
		if (result == Constant.SEARCHLYRIC) {
			...
			if (list_Lyric == null) showDialog(NO_LYRIC);
			else {
		...
		if (result == Constant.SELECTLYRIC) {
			if(lyric_Task.downloadFileSuccess){
				...
				Toast.makeText(this, "Tải lyric không thành công!", Toast.LENGTH_LONG).show();
			}
		}
		return 0;
	}
